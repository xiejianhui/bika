<template>
	<view class="pageBg ">
		<view class="background-white bdb list" v-for="(item,index) in list" :key='item.id'>
			...
			<view class="betweenBox pdl20 pdr20 pdb20">
				...
				<text class="fs26">转后余额:{{item.afterNum}}</text>
			</view>
	...
	export default {
		...
		methods:{
			getList(){
				...
				this.apiUrl.member_balanceTransferPagination({data:{
